...
const HeroTemplate: React.FC<{ title: string; subtitle?: string; s?: DCStyleOverrides }> = ({ title, subtitle, s = {} }) => {
              ...
              {subtitle && (
                <div style={{
                  ...
                  <span style={{
                    ...
                    color: C.gray600, lineHeight: 1.6,
                  }}>
                    {subtitle}
                  </span>
                </div>
...
const MetricTemplate: React.FC<{ value: string; label?: string; subtitle?: string; s?: DCStyleOverrides }> = ({ value, label, subtitle, s = {} }) => {
  const frame = useCurrentFrame();
  ...
  return (
    <AbsoluteFill>
      <GradientBackground colors={[s.backgroundColor ?? C.gray50, C.white, C.blue50]} angle={180} />
      ...
      <SafeArea paddingHorizontal={s.contentPadding ?? 60} paddingVertical={s.contentPadding ?? 60}>
        <AbsoluteFill style={{ justifyContent: "center", alignItems: "center" }}>
          <div style={{
            background: C.white, borderRadius: s.cardBorderRadius ?? 28,
            ...
            <svg width={340} height={340} style={{ position: "absolute", top: "50%", left: "50%", transform: "translate(-50%, -55%)" }}>
              <circle cx={170} cy={170} r={150} fill="none" stroke={C.gray100} strokeWidth={8} />
              <circle
                cx={170} cy={170} r={150}
            ...
            <div style={{ position: "relative", zIndex: 1 }}>
              <div style={{
                ...
                color: s.titleColor ?? C.gray900, letterSpacing: -3, lineHeight: 1,
              }}>
                {value}
              </div>
              {label && (
          ...
          {subtitle && (
            <Sequence from={15}>
              <div style={{
                ...
                <AnimatedTitle text={subtitle} fontSize={s.subtitleFontSize ?? 20} fontWeight={400} color={C.gray600} enterAnimation="fade" enterDuration={15} holdDuration={60} exitDuration={0} />
              </div>
            </Sequence>
          )}
        </AbsoluteFill>
...
const ComparisonTemplate: React.FC<{ before?: string[]; after?: string[]; title: string; s?: DCStyleOverrides }> = ({ before = [], after = [], title, s = {} }) => {
  const frame = useCurrentFrame();
  const { fps } = useVideoConfig();
  ...
  return (
    <AbsoluteFill>
      <GradientBackground colors={[s.backgroundColor ?? C.gray50, C.white]} angle={180} />
      {(s.showParticles !== false) && <ParticleField count={s.particleCount ?? 12} color={s.particleColor ?? `${C.blue600}10`} speed={0.2} direction="up" />}
      ...
      <div style={{
        ...
        <div style={{ textAlign: "center", marginBottom: s.headerMarginBottom ?? 32, flexShrink: 0 }}>
          ...
          <div style={{
            ...
            color: s.titleColor ?? C.gray900, lineHeight: 1.2, letterSpacing: -0.5,
            opacity: interpolate(frame, [3, 14], [0, 1], { extrapolateRight: "clamp" }),
            transform: `translateY(${interpolate(frame, [3, 14], [8, 0], { extrapolateRight: "clamp" })}px)`,
          }}>
            {title}
          </div>
        </div>
        ...
        <div style={{
          ...
          {/* Before card */}
          <div style={{
            flex: 1, background: C.white, borderRadius: cRadius,
            padding: `${cPad}px ${cPad - 2}px`,
            border: `2px solid #FEE2E2`,
            boxShadow: "0 10px 40px -10px rgba(239, 68, 68, 0.08), 0 4px 12px -4px rgba(0,0,0,0.04)",
            transform: `translateX(${interpolate(slideProgress, [0, 1], [-30, 0])}px)`,
            opacity: interpolate(frame, [5, 18], [0, 1], { extrapolateRight: "clamp" }),
            display: "flex", flexDirection: "column",
          }}>
            <div style={{
              display: "inline-flex", padding: `${(s.badgeFontSize ?? 12) - 7}px 14px`, borderRadius: 9999,
              backgroundColor: "#FEE2E2", fontSize: s.badgeFontSize ?? 12, fontWeight: 700,
              color: "#991B1B", letterSpacing: 1.5, marginBottom: cPad - 8,
              fontFamily: FONT_FAMILIES.heading, alignSelf: "flex-start",
            }}>
              ANTES
            </div>
            <div style={{ flex: 1, display: "flex", flexDirection: "column", justifyContent: "center", gap: iGap }}>
              {before.map((item, i) => (
                <div key={i} style={{
                  ...
                  <span style={{
                    fontSize: itemSize, color: s.itemColor ?? C.gray700, fontFamily: FONT_FAMILIES.body,
          ...
          <div style={{
            flex: 1, background: C.white, borderRadius: cRadius,
            padding: `${cPad}px ${cPad - 2}px`,
            border: `2px solid ${C.green100}`,
            boxShadow: `0 10px 40px -10px rgba(22, 163, 74, 0.08), 0 4px 12px -4px rgba(0,0,0,0.04)`,
            transform: `translateX(${interpolate(slideProgress, [0, 1], [30, 0])}px)`,
            ...
            <div style={{
              display: "inline-flex", padding: `${(s.badgeFontSize ?? 12) - 7}px 14px`, borderRadius: 9999,
              backgroundColor: C.green100, fontSize: s.badgeFontSize ?? 12, fontWeight: 700,
              color: "#166534", letterSpacing: 1.5, marginBottom: cPad - 8,
              fontFamily: FONT_FAMILIES.heading, alignSelf: "flex-start",
            }}>
              AHORA
            </div>
            <div style={{ flex: 1, display: "flex", flexDirection: "column", justifyContent: "center", gap: iGap }}>
              {after.map((item, i) => (
                <div key={i} style={{
                  ...
                  <span style={{
                    fontSize: itemSize, color: s.itemColor ?? C.gray900, fontFamily: FONT_FAMILIES.body,
                    lineHeight: 1.4, fontWeight: 500,
                  }}>
                    {item}
                  </span>
...
const TipsTemplate: React.FC<{ title: string; tips: string[]; s?: DCStyleOverrides }> = ({ title, tips = [], s = {} }) => {
  const frame = useCurrentFrame();
  ...
  return (
    <AbsoluteFill>
      <GradientBackground colors={DC_BRAND.gradients.hero} angle={160} />
      {(s.showParticles !== false) && <ParticleField count={s.particleCount ?? 15} color={s.particleColor ?? `${C.orange500}10`} speed={0.25} direction="up" />}
          ...
          {tips.slice(0, 5).map((tip, i) => {
            ...
            return (
              <div key={i} style={{
                ...
                background: C.white, borderRadius: s.cardBorderRadius ?? 18,
...
const DashboardTemplate: React.FC<{ metrics: Array<{ label: string; value: string }>; title: string; s?: DCStyleOverrides }> = ({ metrics = [], title, s = {} }) => {
  const frame = useCurrentFrame();
  const sparkData = [
      ...
      <div style={{
        ...
        <div style={{ marginBottom: s.headerMarginBottom ?? 28, flexShrink: 0 }}>
          ...
          <div style={{
            ...
            color: s.titleColor ?? C.gray900, lineHeight: 1.2,
            opacity: interpolate(frame, [3, 14], [0, 1], { extrapolateRight: "clamp" }),
          }}>
            {title}
          </div>
        ...
        <div style={{
          ...
          {metrics.slice(0, 6).map((m, i) => {
            ...
            return (
              <div key={i} style={{
                width: cardWidth,
                background: C.white, borderRadius: s.cardBorderRadius ?? 20,
                ...
                <div style={{
                  fontSize: lblSize, fontFamily: FONT_FAMILIES.heading, color: C.gray600,
                ...
                <div style={{
                  fontSize: valSize, fontFamily: FONT_FAMILIES.mono, fontWeight: 700,
                  color: s.titleColor ?? C.gray900, marginBottom: 12, lineHeight: 1,
                }}>
                  {m.value}
                </div>
...
const StatementTemplate: React.FC<{ title: string; subtitle?: string; s?: DCStyleOverrides }> = ({ title, subtitle, s = {} }) => {
  const frame = useCurrentFrame();
  const { fps } = useVideoConfig();
  const fadeOpacity = interpolate(frame, [0, 20], [0, 1], { extrapolateRight: "clamp" });
  const scaleVal = spring({ fps, frame, config: { damping: 18, stiffness: 60 } });
  ...
  return (
    <AbsoluteFill>
      <GradientBackground colors={DC_BRAND.gradients.hero} angle={135} />
      {(s.showParticles !== false) && <ParticleField count={s.particleCount ?? 18} color={s.particleColor ?? `${C.blue600}10`} speed={0.2} direction="up" />}
      ...
      <SafeArea paddingHorizontal={pad} paddingVertical={pad}>
        <AbsoluteFill style={{ justifyContent: "center", alignItems: "center" }}>
          <div style={{
            textAlign: "center", opacity: fadeOpacity,
            transform: `scale(${interpolate(scaleVal, [0, 1], [0.95, 1])})`,
            ...
            <div style={{
              fontSize: titleSize, fontFamily: FONT_FAMILIES.heading, fontWeight: 700,
              color: s.titleColor ?? C.gray900, lineHeight: 1.35,
            }}>
              {title}
            </div>
            {subtitle && (
              <div style={{
                ...
                color: C.gray600, marginTop: 24, lineHeight: 1.6,
              }}>
                {subtitle}
              </div>
            )}
...
export const DCPostVisual: React.FC<DCPostVisualProps> = (props) => {
  ...
  const { template = "hero", title, subtitle, tips, dashboardMetrics, metricValue, metricLabel, comparisonBefore, comparisonAfter, styles: s } = props;

  switch (template) {
    case "hero":
      return <HeroTemplate title={title} subtitle={subtitle} s={s} />;
    case "metric":
      return <MetricTemplate value={metricValue || title} label={metricLabel || subtitle} subtitle={subtitle} s={s} />;
    case "comparison":
      return <ComparisonTemplate before={comparisonBefore} after={comparisonAfter} title={title} s={s} />;
    case "tips":
      return <TipsTemplate title={title} tips={tips || []} s={s} />;
    case "dashboard":
      return <DashboardTemplate metrics={dashboardMetrics || [{ label: "KPI", value: title }]} title={title} s={s} />;
    case "statement":
      return <StatementTemplate title={title} subtitle={subtitle} s={s} />;
    default:
      return <HeroTemplate title={title} subtitle={subtitle} s={s} />;
  }
};
